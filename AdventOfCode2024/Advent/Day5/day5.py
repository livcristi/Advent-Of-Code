...
def get_middle_correct(rules, updates):
    return sum(
        update[len(update) // 2] for update in updates if is_update_valid(rules, update)
    )
...
def get_middle_wrong(rules, updates):
    return sum(
        ...
        if not is_update_valid(rules, update) and correct_update(rules, update) is None
    )
...
with open(filename) as f_inp:
    ...
    updates_data = [
        list(map(int, [entry for entry in line.split(",")]))
        for line in f_updates.split("\n")
    ]
